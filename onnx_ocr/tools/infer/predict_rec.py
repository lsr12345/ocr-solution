...
logger = get_logger(name='rec', log_file='./logs/ocr.log')
...
class TextRecognizer(object):
    def __init__(self, args):
        ...
        self.model = InferenceSession(args['rec_model_dir'])
    ...
    def __call__(self, img_list):
        ...
        for beg_img_no in range(0, img_num, batch_num):
            ...
            if self.rec_algorithm == "SRN":
                ...
                #print('input_tensor sp', type(norm_img_batch))
                #self.predictor.run()
                # output_tensors = self.model.run(output_names=None, input_feed={'x': norm_img_batch})
                output_tensors = self.model.run(None, {self.model.get_inputs()[0].name: norm_img_batch})
                # logger.info('time of each rec: %.4fs' % (time.time() - starttime))
                outputs = []
